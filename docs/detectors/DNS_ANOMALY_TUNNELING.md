...
| `minimum_label_octet_diversity_ratio` | `Ratio` | `1/3` (0.333...) | `0..=1` | Threshold for maximum label octet diversity ratio across labels. |

### 2.3 Evaluation Logic

1. Filters for complete `ProtocolKind::Dns` observations representing query messages (`completeness.is_complete()` and `message_kind == DnsMessageKind::Query` or `flags.qr == false`).
2. Computes total expanded wire length ($L_{wire}$), maximum label length ($L_{label,max}$), and maximum label octet diversity ratio ($R_{div,max}$) for each question in the message.
3. A question matches if ALL three conditions are satisfied:
   - $L_{wire} \ge \text{minimum\_qname\_wire\_length}$
   - AND $L_{label,max} \ge \text{minimum\_label\_length}$
   - AND $R_{div,max} \ge \text{minimum\_label\_octet\_diversity\_ratio}$
4. Emits a finding draft containing one `EvidenceKind::ProtocolObservation` record with ordered measurements:
...
2. Evaluates each DNS query observation:
   - Evaluates whether at least one question matches the shared query-shape rule ($L_{wire} \ge \text{min\_qname} \land L_{label} \ge \text{min\_label} \land R_{div} \ge \text{min\_div}$).
   - If matched, marks query observation as a candidate.
...
5. Each finding contains one `EvidenceKind::RatioComparison` record referencing the flow and boundary candidate observations (first and last), with ordered measurements:
